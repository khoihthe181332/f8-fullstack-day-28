...
// Hàm gửi request đến API - chuyển sang Promise
...
// Hàm xử lý tìm user by ID - cập nhật để sử dụng async/await
async function handleSearchUserByID() {
    ...
    try {
        const data = await sendRequest("GET", "https://jsonplaceholder.typicode.com/users");
...
// Thêm chức năng nhấn Enter cho bài 1
...
async function handleShowPost() {
    ...
    try {
        const data = await sendRequest("GET", "https://jsonplaceholder.typicode.com/posts?_limit=5");
...
async function handleShowComment(e) {
    ...
    if (commentsContainer && commentsContainer.innerHTML.trim() !== '') {
        ...
        try {
            const data = await sendRequest("GET", `https://jsonplaceholder.typicode.com/posts/${postId}/comments?_limit=3`);
...
let numberOfPost = 5;
loadMorePostBtn.addEventListener("click", async () => {
    numberOfPost += 3;
    $("#posts-loading").style.display = "block";

    try {
        const data = await sendRequest("GET", `https://jsonplaceholder.typicode.com/posts?_limit=${numberOfPost}`);
        $("#posts-loading").style.display = "none";
        renderPost(data);
    } catch (error) {
        $("#posts-loading").style.display = "none";
...
async function handleLoadTodoByUserId() {
...
// Thêm chức năng nhấn Enter cho bài 3
todoUserIdInput.addEventListener("keypress", function (e) {
    if (e.key === "Enter") {
        handleLoadTodoByUserId();
    }
});
