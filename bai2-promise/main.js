...
function handleShowPost() {
    ...
    sendRequest("GET", "https://jsonplaceholder.typicode.com/posts?_limit=5")
        .then(data => {
...
function handleShowComment(e) {
    ...
    if (commentsContainer && commentsContainer.innerHTML.trim() !== '') {
        ...
        sendRequest("GET", `https://jsonplaceholder.typicode.com/posts/${postId}/comments?_limit=3`)
            .then(data => {
...
loadMorePostBtn.addEventListener("click", () => {
    ...
    sendRequest("GET", `https://jsonplaceholder.typicode.com/posts?_limit=${numberOfPost}`)
        .then(data => {
            $("#posts-loading").style.display = "none";
            renderPost(data);
        })
